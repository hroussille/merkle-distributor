...
contract MerkleDistributorWithDeadline is MerkleDistributor, Ownable {
    ...
    function withdraw() external onlyOwner {
        if (block.timestamp < endTime) revert IMerkleDistributor.NoWithdrawDuringClaim();
        /// DeleteExpressionMutation(`IERC20(token).safeTransfer(msg.sender, IERC20(token).balanceOf(address(this)))` |==> `assert(true)`) of: `IERC20(token).safeTransfer(msg.sender, IERC20(token).balanceOf(address(this)));`
        assert(true);
    }
}