...
contract MerkleDistributorWithDeadline is MerkleDistributor, Ownable {
    ...
    function claim(uint256 index, address account, uint256 amount, bytes32[] calldata merkleProof) public override {
        /// IfStatementMutation(`block.timestamp > endTime` |==> `false`) of: `if (block.timestamp > endTime) revert IMerkleDistributor.ClaimWindowFinished();`
        if (false) revert IMerkleDistributor.ClaimWindowFinished();
