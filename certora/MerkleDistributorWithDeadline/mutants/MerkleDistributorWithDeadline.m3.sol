...
contract MerkleDistributorWithDeadline is MerkleDistributor, Ownable {
    ...
    function withdraw() external onlyOwner {
        /// IfStatementMutation(`block.timestamp < endTime` |==> `false`) of: `if (block.timestamp < endTime) revert IMerkleDistributor.NoWithdrawDuringClaim();`
        if (false) revert IMerkleDistributor.NoWithdrawDuringClaim();
